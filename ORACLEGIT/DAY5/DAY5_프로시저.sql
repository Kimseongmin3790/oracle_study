...
CREATE OR REPLACE PROCEDURE UPDATE_EMP_SAL
    (P_EMPNO IN EMP.EMPNO%TYPE, P_INC IN NUMBER) -- EMP테이블의 EMPNO와 같은 타입으로 선언
IS
BEGIN
    UPDATE EMP
    SET
        SAL = SAL * P_INC -- 급여의 10% 인상
    WHERE EMPNO = P_EMPNO;
    COMMIT;
END;
/
...
EXECUTE UPDATE_EMP_SAL(7499, 1.1);
...
CREATE OR REPLACE PROCEDURE UPDATE_EMP_SAL
    (P_EMPNO IN EMP.EMPNO%TYPE, P_INC IN NUMBER)
IS
    P_COUNT NUMBER;
BEGIN
    UPDATE EMP
    ...
    ELSE
        DBMS_OUTPUT.PUT_LINE('2개이상이 저장되었습니다.'); -- PK가 조건이므로 실행될 일 없어야함           
    END IF;
