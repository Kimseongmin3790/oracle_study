...
FROM STUDENT
UNION
SELECT COUNT(*) -- 컬럼의 타입이 다르기 때문에 불가능
FROM STUDENT;

SELECT STU_NAME 별칭
FROM STUDENT
UNION
SELECT STU_DEPT -- 컬럼의 타입이 같기 때문에 가능
FROM STUDENT;

SELECT STU_NAME
FROM STUDENT
UNION
SELECT STU_NAME, STU_DEPT -- 컬럼의 개수가 다르므로 불가능
FROM STUDENT;

----------------------------------------------------
SELECT * FROM ENROL;

SELECT STU_NAME, AVG(ENR_GRADE) 평균, 1 AS ORDERKEY
FROM STUDENT S
...
SELECT DNAME 부서명, 평균
FROM (
    SELECT DNAME, ROUND(AVG(SAL)) 평균, 1 AS ORDERKEY
    FROM EMP E
