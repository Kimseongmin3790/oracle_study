...
SELECT NAME, COUNT(SUBSTR(NAME, 1, 1)) 같은성씨
FROM CUSTOMER C
...
SELECT NAME, NVL(PHONE, '연락처없음') 전화번호 -- DECODE(PHONE, NULL, '연락처없음', PHONE)
FROM ORDERS O
...
FROM ORDERS O 
INNER JOIN (
    SELECT AVG(SALEPRICE) AVGALL
    FROM ORDERS
) T ON O.SALEPRICE < T.AVGALL;
...
FROM ORDERS O
INNER JOIN (
    SELECT MAX(SALEPRICE) 최고금액
    FROM CUSTOMER C
    INNER JOIN ORDERS O ON C.CUSTID = O.CUSTID
    WHERE C.CUSTID = 3
) T ON O.SALEPRICE > T.최고금액;

-- 13. 이상미디어의 고객별 판매액을 보이시오(고객이름과 고객별 판매액 출력).
SELECT NAME, SUM(SALEPRICE) 판매액
FROM BOOK B
...
SELECT DNAME 부서명, ROUND(AVG(GRADE),2) 평균급여등급
FROM EMP E
...
SELECT E.ENAME, S.GRADE 본인등급, T.GRADE 매니저등급, T.GRADE-S.GRADE 차이
FROM EMP E
INNER JOIN SALGRADE S ON E.SAL BETWEEN S.LOSAL AND S.HISAL
INNER JOIN (
    SELECT E2.EMPNO, GRADE
    FROM EMP E
    INNER JOIN EMP E2 ON E.MGR = E2.EMPNO
    INNER JOIN SALGRADE S ON E2.SAL BETWEEN S.LOSAL AND S.HISAL
) T ON E.MGR = T.EMPNO
GROUP BY E.ENAME, S.GRADE, T.GRADE;

-- 3. 본인 부서의 평균 급여보다 높은 급여를 받는 사람의 이름, 부서명, 급여, 본인부서의 평균급여를 출력하시오.
SELECT ENAME, DNAME, SAL, 평균급여
FROM EMP E
INNER JOIN (
    SELECT E.DEPTNO, DNAME, ROUND(AVG(SAL), 1) 평균급여
    FROM EMP E
    INNER JOIN DEPT D ON E.DEPTNO = D.DEPTNO
...
SELECT NAME, DECODE(ROUND(AVG(CNT)), NULL, '게시글 없음', AVG(CNT)) 조회수평균
FROM TBL_USER U
...
SELECT STU_NAME 이름, AVG(ENR_GRADE) 평균점수, T.평균 학과평균
FROM STUDENT S
INNER JOIN ENROL E ON S.STU_NO = E.STU_NO
INNER JOIN (
...
GROUP BY S.STU_NO, STU_NAME, T.평균
HAVING AVG(ENR_GRADE) > T.평균;
